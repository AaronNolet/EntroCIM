...
if systemctl cat skyspark.service > /dev/null 2>&1; then
  ...
  FinSkyfan=$(cat /etc/init.d/entrocim |grep HomeFolder=/|sed 's/HomeFolder=\///')/lib/fan
  FinSkyhome=$(cat /etc/init.d/entrocim |grep HomeFolder=/|sed 's/HomeFolder=\///')
  HeapSize=$(cat /etc/init.d/entrocim |grep HeapSize=|sed 's/HeapSize=/Fin Stack Heap Size: /' |sed 's/"//g')
  FinSkystatus=$(/usr/sbin/service entrocim status|grep 'Active: ' |sed 's/Active: /EntroCIM Service Status: /'|sed 's/   //')
fi
